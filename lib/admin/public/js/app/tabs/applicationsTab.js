        ...
        [
            {
                ...
                {
                    this.manageApplications("/start_application", "PUT");
                }, this)
            },
            {
                text: "Stop",
                click: $.proxy(function()
                {
                    this.manageApplications("/stop_application", "PUT");
                }, this)
            },
            {
                text: "Restart",
                click: $.proxy(function()
                {
                    this.manageApplications("/restart_application", "PUT");
                }, this)
            }
        ]);
...
ApplicationsTab.prototype.getTableData = function(results)
{
    ...
    var routes        = results[4].response.items;
    ...
    for (var applicationIndex in applications)
    {
        ...
        if (organization != null && space != null)
        {
            ...
            row[0] = (organization.name + "/" +space.name + "/" + application.name)
        }
        else
        {
            Utilities.addEmptyElementsToArray(row, 1);
        ...
        for (var applicationIndex in applications)
        {
            ...
            for (instanceIndex in application)
            {
                ...
                if (row == null)
                {
                    ...
                    {
                        row.push(organization.name + "/" + space.name);
                        row[0] = organization.name + '/' + space.name + '/' + instance.application_name
                    }
                    else
                    {
                        Utilities.addEmptyElementsToArray(row, 1);
                ...
                else
                {
                    ...
                    if (instance.state_running_timestamp != null)
                    {
                        row[4] = instance.state_running_timestamp * 1000;
                    }

                    row[ 5] = instance.application_uris;
                    row[ 7] = instance_index;
                    row[ 8] = instance.services.length;
                    row[ 9] = (instance.used_memory_in_bytes != null) ? Utilities.convertBytesToMega(instance.used_memory_in_bytes) : 0;
                    row[10] = (instance.used_disk_in_bytes   != null) ? Utilities.convertBytesToMega(instance.used_disk_in_bytes)   : 0;
                    row[11] = (instance.computed_pcpu        != null) ? (instance.computed_pcpu * 100)                              : 0;
                    row[15] = host;
                    row[16] = deaIndex;

                    // Need the specific instance for this row
                    row[17] = {
                                  "application": row[17].application,
                                  "instance": instance,
                                  "space": row[17].space,
                                  "organization": row[17].organization
                              };
                    row.push(0)
                }
            }
...
ApplicationsTab.prototype.clickHandler = function()
{
    ...
    if (selected.length > 0)
    {
        ...
        if (row[4] != "")
        {
            this.addPropertyRow(table, "Started", Format.formatDateNumber(row[4]));
        }
        ...
        if (row[6] != "")
        {
            var buildpackArray = row[6].split(",");
        ...
        if (row[7] !== "")
        {
            this.addPropertyRow(table, "Instance Index", Format.formatNumber(row[7]));
        }
        ...
        if (row[8] !== "")
        {
            this.addPropertyRow(table, "Services Used", Format.formatNumber(row[8]));
        }
 
        this.addPropertyRow(table, "Memory Used", Format.formatNumber(row[ 9]));
        this.addPropertyRow(table, "Disk Used",   Format.formatNumber(row[10]));
        this.addPropertyRow(table, "CPU Used",    Format.formatNumber(row[11]));
        ...
        if (space != null)
        {
            ...
            $(spaceLink).click(function()
            {
                ...
                AdminUI.showSpace(Format.formatString(row[14]));
        ...
        if (row[15] != "")
        {
            var dea = Format.formatString(row[15]);
...
ApplicationsTab.prototype.manageApplications = function(path, method)
{
    ...
    {
        alert("Error handling the follwoing applications:\n" + error_apps);
    }
    else
    {
        // Todos: we need to implement a polling backend service to get the latest app data in the future
        alert("The operation is finished without errors.\n\nNOTE: \nDue to the VARZ cache, the detail info of the apps maynot be update to date.\nPlease refresh the page later to gain the latest result");
    }
